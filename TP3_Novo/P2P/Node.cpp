...
void Node::addNode(int indice, string node) {
    if(indice == nodes.size())
        nodes.push_back(node);
    else
        nodes.insert(nodes.begin()+indice, node);
}
...
int Node::getEnderecoInicial() {
    int end = 65 + (getIndice() * ceil(26.0 / getNumNodes()));
    return end;
}

int Node::getEnderecoFinal() {
    int end = min((int) (65 + ((getIndice() + 1) * ceil(26.0 / getNumNodes())) - 1), 90);
    return end;
}
